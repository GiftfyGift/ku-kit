...
function generatePoRequestPdf(order, pr) {
  ...
  // Only one Date field on the document — it's already up top next to the
  // PO number, so this line isn't a second date field, just the buyer-side
  // contact's name against a signature-style rule (the underscores are the
  // line, not a blank to fill in — the name goes directly above them).
  y += 20;
  doc.text(order.buyer.contact || '', marginX, y);
  y += 3;
  ...
  y += 11;
  doc.text('Your assigned sales rep is notified automatically; you\'ll hear back once this order is reviewed.', marginX, y);

  // Official letterhead footer, matching Siam Kubota's own document
